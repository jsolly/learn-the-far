...
export function verifyResponse({ url, status, release }, canonical, expected, git) {
...
export async function runSmoke({
	...
	try {
		...
		git("git", ["merge-base", "--is-ancestor", receipt.expectedSha, "origin/main"]);
		...
		browser = await (launch ?? (async () => (await import("playwright")).chromium.launch()))();
			...
			if (
				...
				documents.push(
					(async () => {
						...
						receipt.observations.push(observation);
						verifyResponse(observation, scenario.productionUrl, receipt.expectedSha, git);
					})().catch(recordError),
				);
			}
		});
		await Promise.race([
			...
			new Promise((_, reject) => {
				timer = setTimeout(() => reject(new Error("Behavior deadline exceeded")), behaviorMs);
			}),
		]);
		...
		if (context) {
			await context.tracing.stop({ path: resolve(artifacts, "trace.zip") }).catch(recordError);
		}
		if (browser) {
