...
import { capturedFillRatio, hubCapturedPercent, sliceAngleValue } from "$lib/pie-progress";
...
function sliceFill(
	...
	if (kind === "learning" && learningRatio === 0) {
		return null;
	}
	if (kind === "mastered" && masteredRatio === 0) {
		return null;
	}

	const outer =
		kind === "learning" ? progressOuterRadius(capturedRatio) : progressOuterRadius(masteredRatio);
	const color =
		kind === "mastered"
...
const attachChart: Attachment<HTMLDivElement> = (element) => {
	...
	const resizeObserver = new ResizeObserver(([entry]) => {
		...
		if (Math.abs(nextWidth - chartWidth) < 0.5) {
			return;
		}
...
let wheelDescription = $derived(
	[`Mastery wheel — ${hubPercent}% captured.`, ...stats.map((stat) => sliceSummary(stat))].join(
		" ",
	),
);
</script>
