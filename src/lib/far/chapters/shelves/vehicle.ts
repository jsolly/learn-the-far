...
export const VEHICLE_SHELF_CHAPTERS: Chapter[] = [
	...
	chapter({
		...
		pieces: [
			...
			},
			{
				...
				teach:
					"An on-ramp is a continuously open vehicle solicitation that lets new offerors seek direct holder access outside the original base award. When a target vehicle is continuously open for on-ramps (for example GSA’s OASIS+ solicitations), treat near-term participation and direct access as two decisions on two calendars: pursue a compliant role under an eligible holder while separately investing in a direct-access proposal. Access workstreams are capital projects; order capture is a different motion.",
				watchFor:
	...
	chapter({
		...
		pieces: [
			...
			},
			{
				...
				teach:
					"Order scope is the work authorized under a specific order, which must remain within the scope, period, and maximum value of the underlying contract. Material additions risk a cardinal change outside the original competition — GAO has treated materially different added services as improper sole-source awards outside the original contract’s scope, with payment and protest risk traveling together. Analyze both base-IDIQ and order layers; obtain contracting-officer direction before performing; steer out-of-scope needs to a competed or otherwise authorized path.",
				watchFor:
	...
	chapter({
		...
		summary: "Defines how GWAC and MAS lanes differ by scope, holders, and ordering mechanics.",
		intro:
